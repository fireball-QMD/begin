...
! Construct r at each mesh point.
        rcp_max = -1.0d0
        do issh = 1, nssh
         rcp_max = max(rcp_max,rcutoff(issh))
        end do
        dr = rcp_max/real(mesh - 1)
        ...
        write (*,*) ' r(mesh) = ', r(mesh), ' must = rcp_max = ', rcp_max
        if (abs(r(mesh) - rcp_max) .gt. 1.0d-6)                              &
...
! term 4 = kinetic
         terms = 0.0d0
         do issh = 1, nssh
          terms(1) = terms(1) + 2.0d0*v_local(issh)
          terms(2) = terms(2) + 2.0d0*uxc_local(issh)
          terms(3) = terms(3) + 2.0d0*vna_local(issh)
          terms(4) = terms(4) + 2.0d0*vkin_local(issh)
         end do
