...
! is provided by the user. Finally, ground state and excited wf are mixed 
! creating new kind of wavefunctions.  
!
! ===========================================================================
! Code written by:
! James P. Lewis
...
! ===========================================================================
!JOM code changed by Jose Ortega to 
!
! (1) generate wavefunctions as a mixture
! of GS and DMOL states
!
! (2)deal with the case
!   Rc(GS) .ne. Rc(DMOL)  : still needs some work !
!
! ===========================================================================
!
! Program Declaration
! ===========================================================================
        subroutine rcatms_GSM (mesh, mesh_gs, rc_max, rcutoff_gs, r,  &
     &                          psi_gs)
        use begin_input
        use constants
        use precision
...
!JOM
        integer, intent (in), dimension (nssh) :: mesh_gs
...
!JOM
        real(kind=long), intent (in), dimension (nssh) :: rcutoff_gs
...
!JOM
        do issh = 1, nssh
         mesh_psirc(issh) = int((rcut_ion(issh) + 1.0d-4)/dr) + 1
...
!        call pp(ppionfile, nssh, mesh, xnz, r, vc, vl, ioptionpp, exmix)
        call pp(ppionfile, mesh, r, vc, vl, ioptionpp, exmix)
...
! siesta formulae, PRB 64, 235111 (2001)
                    vXe(issh,ipoint) =                                      &
     &                 Vo(jssh)*exp( -1.0d0*(rcutoff(issh) - r0(jssh))      &
...
! Check that int[0,rc] sig(r) dr = z
        ...
        if (nznuc .ne. 1 .and. difference .gt. tolerance)                    &
     &   stop ' Check: Sum of charge density - not equal to nzval_ion '
...
! Solve Schroedinger equation for potential v
      ...
      &               eout, psi0)
!JOM-test
!          call psirc (mesh_psirc(issh), 1, l, rcutoff_psirc(issh), v,        &
!      &               eout, psi0)
...
!JOM WARNING: for future work with Rc(gs) .ne. Rc(DMOL), BE CAREFUL with mesh
...
!          integral =                                                         &
!           integral + rmult(ipoint)*psi_gs(issh,ipoint)*psi_gs(issh,ipoint)
!         end do
!	write (*,*) 'OVERLAP GS-GS', integral
!         integral = 0.0d0
!         do ipoint = 1, mesh_new
!          integral =                                                         &
!           integral + rmult(ipoint)*psi(issh,ipoint)*psi(issh,ipoint)
!         end do
!	write (*,*) 'OVERLAP EX-EX', integral
...
!JOM-test
	...
	write (*,*) 'OVERLAP GS-DMOL', integral


! ***************************************************************************
!JOM MIX GS and DMOL wavefunctions to OPTIMIZE basis set !
! mix as psi_gs = c1*psi_gs + (1-c1)*psi_DMOL   (psi_DMOL = psi)
! and orthonormalize
! ***************************************************************************

!JOM : mix GS and DMOL
         do ipoint = 1, mesh_new
          psi_new(issh,ipoint) = cmix(issh)*psi_gs(issh,ipoint)                &
   &                              + (1.0d0-cmix(issh))*psi(issh,ipoint)
         end do
...
!JOM : orthonormalize
         ...
         do ipoint = 1, mesh_new
          integral =                                                         &
           integral + rmult(ipoint)*psi_new(issh,ipoint)*psi_new(issh,ipoint)
         end do
	write (*,*) 'NEW OVERLAP GS-GS', integral
	
         psi_new(issh,1:mesh_new) = psi_new(issh,1:mesh_new)/sqrt(integral)

!JOM : check orthonormal
         integral = 0.0d0
         do ipoint = 1, mesh_new
          integral =                                                         &
...
!         end do
!!JOM
!	 if (mesh_gs(issh) .gt. mesh_new) then
!         do ipoint = mesh_new + 1, mesh_gs(issh)
!          psi_new(issh,ipoint) =  - integral*psi_gs(issh,ipoint)
!         end do
!	 end if
!	 if (mesh_psirc(issh) .gt. mesh_new) then
!         do ipoint = mesh_new + 1, mesh_psirc(issh)
!          psi_new(issh,ipoint) = psi(issh,ipoint)
!         end do
!	 end if
!
!! Check normalization
...
!JOM : now write the OPTIMAZED (MIX) orbitals into the "GS STATE" FILE"
!
! ***************************************************************************
...
!
! JOM plot original excited states (without orthogonalization)
!dani         if (l .eq. 0) open (unit = 18, file = 'sstate10', status = 'unknown')
!dani         if (l .eq. 1) open (unit = 18, file = 'pstate10', status = 'unknown')
!dani         if (l .eq. 2) open (unit = 18, file = 'dstate10', status = 'unknown')
!dani         if (l .eq. 3) open (unit = 18, file = 'fstate10', status = 'unknown')
!
! JOM comment: in these plots the units are not consistent: r(a.u.),yy(A^(-1.5))
!
!JOM
! JOM plot OPTIMIZED  states
	mesh_new = max(mesh_gs(issh),mesh_psirc(issh))
!dani         do ipoint = 1, mesh_new
!dani          write (17,401) r(ipoint), yy(ipoint)
!          write (17,401) r(ipoint), psi_new(issh,ipoint)
!dani         end do
!
! JOM plot DMOL  states
!dani         do ipoint = 2, mesh_new
!dani          write (18,401) r(ipoint), psi(issh,ipoint)/(abohr15*r(ipoint))
!          write (18,401) r(ipoint), psi(issh,ipoint)
!dani         end do
!dani         close (unit = 17)
!dani         close (unit = 18)
        end do
