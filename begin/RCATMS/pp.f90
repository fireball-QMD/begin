...
! Program Declaration
! ===========================================================================
        subroutine pp (ppfilein, mesh, r, vc, vl, ioptionpp, exmix)
        use begin_input
        use constants
        use pp_storage
        use precision
...
!       real(kind=long), intent (in) :: xnz
        ...
        character(len=8), intent (in) :: ppfilein
...
! Find out the mesh size for the non-local piece of the pseudopotential.
        call get_nlmesh_size (ppfilein, nlmesh)

! Read in the psuedopotential
        write (*,*) ' open file ', ppfilein
        open (unit = 88, file = ppfilein, status = 'old')
